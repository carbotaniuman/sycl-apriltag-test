    ...
    auto init_event = q.submit([=](sycl::handler &h) {
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> it) {
                ...
                uint16_t local_label = static_cast<uint16_t>(local_label_img & LABEL_VALUE_MASK);

                bool local_label_too_small = sizes[local_label].value < 25;
                uint16_t test_local = static_cast<uint16_t>(local_label_img & LABEL_PIXEL_MASK);

                auto handle_pixel_test = [&](int pixel_dy, int pixel_dx,
                                             size_t point_offset,
                                             HalfPixel half_pixel) {
                    uint16_t test_label_img =
                        shared_ptr[local_linear_id + pixel_dy * local_width +
                                   pixel_dx];

                    uint16_t test_other = static_cast<uint16_t>(test_label_img & LABEL_PIXEL_MASK);
                ...
                {
                    uint16_t left_label_img = shared_ptr[local_linear_id - 1];
                    uint16_t test_left = static_cast<uint16_t>(left_label_img & LABEL_PIXEL_MASK);
                    uint16_t down_label_img =
                        shared_ptr[local_linear_id + local_width];
                    uint16_t test_down = static_cast<uint16_t>(down_label_img & LABEL_PIXEL_MASK);
