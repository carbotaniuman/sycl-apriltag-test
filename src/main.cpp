...
int main(int argc, char *argv[]) {
    ...
    auto [keys_end, values_end] = oneapi::dpl::reduce_by_segment(
        ...
        transform_values, trash_keys_buffer, values_start, std::equal_to<>(),
        [](const auto &left, const auto &right) {
    ...
    if (false) {
        ...
        for (size_t i = 0; i < std::distance(values_start, values_end); i++) {
            ...
            stbi_write_png(buffer, width, height, 3, cluster_image, width * 3);
    ...
    if (false) {
        ...
        for (int f = 0;
            ...
            std::cout << "start at " << xt.start << " with count " << xt.count
                      << std::endl;
    ...
    if (debug) {
        ...
        for (int i = 0; i < cluster_data_new_count; i++) {
            ...
            for (int j = 0; j < peak_extent.count; j++) {
                ...
                auto x =
                    filtered_cluster_points[extents.start +
                                            corner_test.line_fit_point_index]
                        .x_value();
                auto y =
                    filtered_cluster_points[extents.start +
                                            corner_test.line_fit_point_index]
                ...
                if (j < 10) {
                    cluster_image[(y * width + x) * 3 + ((i + 1) % 3)] = 255;
                } else {
    ...
    if (false) {
        ...
        for (int i = 0; i < cluster_data_new_count; i++) {
            ...
            for (int j = 0; j < peak_extent.count; j++) {
                ...
                auto x =
                    filtered_cluster_points[xt.start +
                                            corner_test.line_fit_point_index]
                        .x_value();
                auto y =
                    filtered_cluster_points[xt.start +
    ...
    if (debug) {
        ...
        for (int i = 0; i < cluster_data_new_count; i++) {
            ...
            const auto &extents = rewritten_filtered_values_buffer[i];
            std::cout << i << " " << quad[0] << " " << quad[1] << " " << quad[2]
                      << " " << quad[3] << std::endl;

            if (quad[0] != 0 || quad[1] != 0 || quad[2] != 0 || quad[3] != 0) {
                for (int i = 0; i < 4; i++) {
                    auto x = filtered_cluster_points[extents.start + quad[i]]
                                 .x_value();
                    auto y = filtered_cluster_points[extents.start + quad[i]]
                                 .y_value();
